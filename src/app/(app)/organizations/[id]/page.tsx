...
export default function OrganizationDetailsPage() {
  ...
  const updateDeliveryAddress = async (addrId: string, field: keyof DeliveryAddress, value: string | null) => {
    // DB Update
    const { error } = await supabase.from('delivery_addresses').update({ [field]: value }).eq('id', addrId)
    if (!error) {
  ...
  const updateOrgField = async (field: keyof Organization, value: string | number | null) => {
    ...
    // Update DB
    const { error } = await supabase.from('organizations').update({ [field]: valForDB }).eq('id', org.id)
    if (error) {
      console.error(error)
